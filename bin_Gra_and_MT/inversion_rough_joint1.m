...
addpath /public/home/caihongzhu/zhuhao/Joint_3parameters/Joint_structured_Inv/green3d/double_aug13_intel_omp/bin
addpath /public/home/caihongzhu/zhuhao/Joint_3parameters/Joint_structured_Inv/hdf5/matlab
%addpath /public/home/caihongzhu/zhuhao/Joint_3parameters/Joint_structured_Inv/joint_for_gravity_and_MT/joint_tidu
% 
clear;
tic;
...
JacobUpdate = 1;
 ...
 t_xyz_center1 =  t_xyz_center_inv;
...
%            Jacob,JacobInv,JacobDet,Gauss3DW,Gauss,EPGauss,compress,symFlag,fileformat,Num_Comm,Q_Ex,Q_Ey,Q_Ez,Q_Hx,Q_Hy,Q_Hz,mpirun,xa,ya,za,minv_ind,Xtop,Ytop,Ztop,inv_flag);
   ...
   A02       = A_gra*p3;
    ...
    save InvResult m m2 dm dm1 dm3 dminv dminv2 Minv Minv2 misfit misfit2 apha apha2 x1 y1 z1 minv_ind invBnd t_xyz_center Lxm1 Lxm3 Lym1 Lym3 Lzm1 Lzm3 Lxyzm1 Lxyzm3 SWm SWm2 Sd Sd2 data_all
    ...
    SG(n+1)                       = GetSGnew(m,m2Log,I);
    SGx(n+1)                      = GetSGnew(m,m2Log,Lx);
    SGy(n+1)                      = GetSGnew(m,m2Log,Ly);
    SGz(n+1)                      = GetSGnew(m,m2Log,Lz);
    SGD(n+1)                      = SGx(n+1)+SGy(n+1)+SGz(n+1);
    
    
    switch GramianType
        case 1
             DG           = GetDGnew(m,m2Log,I);
            % Calculate regularization parameter
             beta(n+1)    = GetJointPar_Ap(J,A02,Wd,Wd2,DG,qbeta,cbeta,n);
             WG_left      = beta(n+1)*DG;
             WG_right     = beta(n+1)*SG(n+1);
        case 2
             DGx                         = GetDGnew(m,m2Log,Lx);
             DGy                         = GetDGnew(m,m2Log,Ly);
             DGz                         = GetDGnew(m,m2Log,Lz);
%             % Calculate regularization parameter
             betax(n+1)                  = GetJointPar_Ap(J,A02,Wd,Wd2,DGx,qbeta,cbeta,n);
             betay(n+1)                  = GetJointPar_Ap(J,A02,Wd,Wd2,DGy,qbeta,cbeta,n);
             betaz(n+1)                  = GetJointPar_Ap(J,A02,Wd,Wd2,DGz,qbeta,cbeta,n);
    ...
    P = -[Wd*real(rn);Wd*imag(rn);Wd2*rn2;WG_right;sqrt(apha(n+1))*(L)*(m-mapr);sqrt(apha2(n+1))*(L)*(m2Log-m2aprLog);MT_right;sqrt(gama2(n+1))*Wm2*(m2Log-m2aprLog)];
    Wm_left  = [blkdiag(sqrt(apha(n+1))*(L),sqrt(apha2(n+1))*(L));blkdiag(MT_left,sqrt(gama2(n+1))*Wm2)];  
    ...
    [dm,flag,relres,iter,resvec,lsvec] = lsqr_Implicit_A_MaxIter_Ap2(A1_dense,A3_dense,Wd,Wd2,WG_left, Wm_left,P,lsqr_tol,lsqr_maxit );
    dm1  = dm(1:Nm);   
    dm3  = dm(Nm+1:end);
    ...
    kn = min(kn)
    kn2   =  Step_Linear_Search_log1(A02,dobs2,m2Log,dm3,m2min,m2max,pp)
    
     if GramianType == 1
        [gkn,gkn2] = Step_Linear_Search_log_joint(m,dm1,mmin,mmax,m2Log,dm3,m2min,m2max,pp)
        kn         = (kn+gkn)/2;
        kn2        = (kn2+gkn2)/2;
        m          = m+kn*dm1;
        m2Log      = m2Log+kn2*dm3;
    else
        [gxkn,gxkn2]   = Step_Linear_Search_log_joint_G(m,dm1,mmin,mmax,m2Log,dm3,m2min,m2max,Lx,pp)
        [gykn,gykn2]   = Step_Linear_Search_log_joint_G(m,dm1,mmin,mmax,m2Log,dm3,m2min,m2max,Ly,pp)
        [gzkn,gzkn2]   = Step_Linear_Search_log_joint_G(m,dm1,mmin,mmax,m2Log,dm3,m2min,m2max,Lz,pp)
